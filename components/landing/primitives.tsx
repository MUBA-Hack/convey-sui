...
export function SplitButton({
  ...
  const inner = (
    <>
      <span className={cn("cv-btn__label cv-micro", stretch && "flex-1 justify-center")}>
        {children}
      </span>
      {chip && (
        <span className="cv-btn__chip" aria-hidden>
          <Arrow />
        </span>
      )}
    </>
...
export function Eyebrow({
  ...
  return (
    <p className={cn("cv-micro relative", className)}>
...
export function CornerPin({ className }: { className?: string }) {
  return (
    <span
      aria-hidden
      className={cn("absolute block size-1.5 bg-[var(--cv-accent)]", className)}
    />
  );
}
...
export function NumberChip({
  ...
  return (
    <span
      className={cn(
        "cv-micro cv-micro-sm inline-flex h-[22px] w-[30px] shrink-0 items-center justify-center gap-[3px]",
        tone === "dark" && "bg-black text-white",
        tone === "light" && "bg-[var(--cv-surface)] text-black",
  ...
  return (
    <div aria-hidden className={cn("pointer-events-none absolute inset-0", className)}>
      {positions.map((left, i) => (
        <span
          key={i}
          className={cn("absolute top-0 bottom-0", dark ? "cv-vr--dark" : "cv-vr")}
...
export function Hairline({ dark = false, className }: { dark?: boolean; className?: string }) {
  return <div aria-hidden className={cn(dark ? "cv-hr cv-hr--dark" : "cv-hr", className)} />;
}
