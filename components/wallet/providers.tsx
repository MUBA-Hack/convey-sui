...
const NETWORKS: WalletNetwork[] = ["localnet", "testnet", "mainnet"];
// JSON-RPC endpoints: Mysten's *.sui.io hosts are unreachable from some
// networks (TLS interception), which silently killed wallet finalization
...
function getDefaultNetwork(value: string | undefined): WalletNetwork {
  return NETWORKS.includes(value as WalletNetwork)
    ? (value as WalletNetwork)
    : "testnet";
}
    ...
    const { unregister } = registerEnokiWallets({
      ...
      providers: {
        google: {
          ...
          // Default redirect is the CURRENT page, so signing in from a deep
          // route sends an unregistered redirect_uri and Google 400s. Pin the
          // one registered URI (origin + "/") for every page.
