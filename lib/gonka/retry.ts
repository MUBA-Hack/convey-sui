/**
 * Visible retry policy for the GonkaRouter commerce adapter.
 *
 * The OpenAI SDK's own retries stay disabled (maxRetries: 0) so every
 * application retry is fully visible in the audit trail. Only timeout, 429,
 * and eligible transient 5xx are retried — at most once.
 */
...
/** Retryable: timeout, 429, and transient 5xx only. */
...
export async function runWithVisibleRetry<T>(
  ...
  for (let retry = 0; retry <= retryLimit; retry += 1) {
    ...
    try {
      ...
      // GonkaRouter guidance asks 429 callers to back off 30–60s.
      const rateLimited = getGonkaErrorStatus(error) === 429;
      const baseDelayMs = Math.max(0, options.baseDelayMs ?? (rateLimited ? 30_000 : 250));
      const jitterMs = Math.max(0, options.jitterMs ?? (rateLimited ? 30_000 : 250));
