...
import {
  PROTECTED_TRANSFER_TERMINAL_VERIFY_MAX_BYTES,
...
import { verifyProtectedTransferTerminalOnSui } from "@/lib/remittance/protected-transfer-terminal.server";
import {
...
function response(body: ProtectedTransferTerminalVerifyResponse) {
  return NextResponse.json(ProtectedTransferTerminalVerifyResponseSchema.parse(body), {
    status: 200,
...
export async function POST(req: Request) {
  ...
  const raw = await readBoundedUtf8Body(req, PROTECTED_TRANSFER_TERMINAL_VERIFY_MAX_BYTES);
  ...
  const parsed = ProtectedTransferTerminalVerifyRequestSchema.safeParse(body);
  ...
  try {
    return response(await verifyProtectedTransferTerminalOnSui(parsed.data));
  } finally {
    releaseProtectedTransferTerminalGate();
  }
}
